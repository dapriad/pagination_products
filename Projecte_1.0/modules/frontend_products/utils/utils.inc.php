...
function paint_template_error($message) {
    $log = Log::getInstance();
    $log->add_log_general("error paint_template_error", "frontend_products", "response" . http_response_code()); //$text, $controller, $function
    $log->add_log_user("error paint_template_error", "", "frontend_products", "response" . http_response_code()); //$msg, $username = "", $controller, $function
...
function paint_template_products($arrData) {
    print ("<script type='text/javascript' src='modules/frontend_products/view/js/modal_products.js' ></script>");
    print ("<section >");
    ...
    if (isset($arrData) && !empty($arrData)) {
        ...
        foreach ($arrData as $product) {
            ...
            print ("<div class='prod' id='".$product['id_prod']."'>");
