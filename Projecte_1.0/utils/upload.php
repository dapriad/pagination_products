...
function upload_files() {
    ...
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/Projecte_1.0/media/'.$_FILES['file']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/Projecte_1.0/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    } 
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = '/Projecte_1.0/media/default-avatar.jpg';
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
...
function remove_files(){
	$name = $_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/Projecte_1.0/media/'.$name)){
		unlink($_SERVER['DOCUMENT_ROOT'].'/Projecte_1.0/media/'.$name);
		return true;
	}else{
		return false;
	}
